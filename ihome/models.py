...
class BaseModel(object):
    '''模型基类，为每个模型补充创建时间与更新时间，自动维护的，在程序里不用管'''
    create_time=db.Column(db.DateTime,default=datetime.now)
    update_time=db.Column(db.DateTime,default=datetime.now,onupdate=datetime.now)
    #onupdate表示这个字段是可以自动更新的，在修改这条记录的时候就会更新这个字段的时间
...
class User(BaseModel,db.Model):
    ...
    id=db.Column(db.Integer,primary_key=True,autoincrement=True)#用户id,可自增
    ...
    id_card=db.Column(db.String(80))#身份证，如果想要上传房屋，就需要实名认证
    avatar_url=db.Column(db.String(128))#用户头像路径
    houses=db.relationship('House',backref='user')#跟用户发布的房屋绑定
    orders=db.relationship('Order',backref='user')#跟用户成交的订单绑定

    #property装饰器，会把函数变为属性，属性名即为函数名
    @property
    def password(self):
        '''读取属性的函数行为'''
        #print(user.password)#比如这样的代码，就会调用这个函数
        #函数的返回值会作为属性值 return ...
        raise AttributeError('这个属性只能设置，不能读取')


    #使用函数名.setter，对应设置属性操作
    @password.setter
    def password(self,origin_password):
        """
        设置属性  user.password='  xxx'
        :param value: 设置属性时的数据value就是‘xxx’,明文密码
        :return:
        """
        self.password_hash=generate_password_hash(password=origin_password,salt_length=4)
...
class Area(BaseModel,db.Model):
    '''城区表,为了在前边检索的时候快一点，而不是直接去访问house表去找哪些城区，因为house表可能很大'''
    __tablename__='ih_area_info'

    id=db.Column(db.Integer,primary_key=True,autoincrement=True)
    name=db.Column(db.String(32),unique=True,nullable=False)#城区名
...
house_facility=db.Table(
    'ih_house_facility',
    db.Column('house_id',db.Integer,db.ForeignKey('ih_house_info.id')),
    db.Column('facility_id',db.Integer,db.ForeignKey('ih_facility_info.id'))
)
...
class Facility(BaseModel,db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)  # 用户id,可自增
    name = db.Column(db.String(32), unique=True, nullable=False)  # 设施名
